const primero = 'select l.id_libro, l.nombre_libro, l.autor_libro, l.editorial, l.isbm, l.codigo_barras, l.cantidad,l.id_categoria , c.descripcion as categoria, l.fecha_publicacion, l.tomo_inicial, l.tomo_final, l.parte, ta.descripcion as tipo_autor, img.imagen, l.codigo_interno, l.estado ';
...
const script = {
    ...
    updateLibro: 'UPDATE LIBRO SET nombre_libro = $1, autor_libro = $2, editorial = $3, isbm = $4, codigo_barras = $5, cantidad = $6, id_categoria = $7, fecha_publicacion = $8, tipo_autor = $10, tomo_inicial = $11, tomo_final = $12, parte = $13, estado = $14, codigo_interno = $15, id_imagen = $16 WHERE id_libro = $9',
    deleteLibro: 'delete from LIBRO where ID_LIBRO = $1',
    selectNombreLibro: 'SELECT LB.ID_LIBRO, LB.NOMBRE_LIBRO, LB.AUTOR_LIBRO, LB.EDITORIAL , LB.ID_CATEGORIA , (CT.DESCRIPCION || \' \' || CT.numero_categoria) AS CATEGORIA , LB.ISBM, LB.CODIGO_BARRAS, LB.CANTIDAD, LB.FECHA_PUBLICACION, img.imagen FROM LIBRO AS LB left JOIN CATEGORIAS AS CT ON LB.ID_CATEGORIA = CT.ID_CATEGORIA left JOIN imagen_libro as img on LB.id_imagen = img.id_imagen WHERE lower(LB.NOMBRE_LIBRO) LIKE $1',
    ...
    update_imagen: 'update imagen_libro set imagen = $1 where id_imagen = (select il.id_imagen from imagen_libro as il join libro as l on l.id_imagen = il.id_imagen where id_libro = $2);',
    categoriasPadre: `select * from categorias c where cat_principal = 1 order by id_categoria asc;`,
    libroCategoria: `select lb.nombre_libro, lb.autor_libro, lb.editorial, lb.isbm, lb.codigo_barras, lb.cantidad, lb.codigo_barras, c.descripcion 
    from libro as lb
    inner join categorias as c 
    on lb.id_categoria = c.id_categoria 
    where c.categoria_padre = $1;`,
    subsecciones: `SELECT * FROM CATEGORIAS where categoria_padre = $1`
}
